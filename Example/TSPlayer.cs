...
*/
		...
		void Update ()
		{
				if (!inside) {//if not showing inside
		...
		{//example GUI for an owned post
				...
				for (int g = 0; g<controller.goods.Count; g++) {//go through all groups
						...
						if (g != 3) {//make sure that not the machinery group
								...
								for (int i = 0; i<controller.goods[g].goods.Count; i++) {//go through all items
										...
										if (GUI.Button (new Rect (760, labelPos * 30, 50, 30), "Unload") && cargo [g] [i] > 0) {//if unload and has cargo
												cargo [g] [i]--;//remove cargo
												nearPost.stock [g].stock [i].number++;//add cargo
										}//end if unloading
								
										if (GUI.Button (new Rect (820, labelPos * 30, 50, 30), "Load") && nearPost.stock [g].stock [i].number > 0) {//if load and has cargo
						...
						} else {//end if not machinery group
								...
								for (int i = 0; i<controller.goods[g].goods.Count; i++) {//for all machinery items
						...
						if (ShowGroup (g, selected)) {//only need to go through items if at least one is enabled
								...
								for (int i = 0; i<controller.goods[g].goods.Count; i++) {//go through all items
										if (ShowItem (g, i, selected)) {//if the item is to be shown
												...
												if (GUI.Button (new Rect (850, labelPos * 30, 50, 30), (selected == 0 ? "Buy" : "Sell"))) {//show a buy/sell button
