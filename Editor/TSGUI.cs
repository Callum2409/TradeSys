...
{//use namespace to stop any name conflicts
    public class TSGUI
    {
        ...
        {//a list of bool options
            ...
            if (showHoriz)
            {//if showing items horizontally
                for (int b = 0; b < option.arraySize; b = b + 2)
                {//add 2 each time because 2 option are being displayed
        ...
        {//a list of labels with two part strings
            ...
            {//if showing items horizontally
                ...
                {//add 2 each time because 2 options are being displayed
                    EditorGUILayout.BeginHorizontal();
        ...
        {//a list with straight up options with no property
            ...
            {//if showing items horizontally
                ...
                {//add 2 each time because 2 option are being displayed
                    ...
                    if (b < option.arraySize - 1)
                    {//show the RH option if is less than length -1
        ...
        {//get all of the item names
            ...
            for (int g1 = 0; g1 < controller.goods.Count; g1++)
        ...
        {//get the tooltip for the needing or making
            ...
            for (int n = 0; n < currentNM.Count; n++)
            {//go through all needing or making
                current = currentNM[n];
        ...
        {//the manufacturing options for posts and traders
            ...
            if (AnyManufacturing(controllerMan, obj, post))
            {
                ...
                for (int m = 0; m < controllerMan.arraySize; m++)
                {//go through manufacturing groups
                    ...
                    if (ShowGroup(cManGC.FindPropertyRelative("manufacture"), obj, post))
                    {//check that something in the group is showing to decide whether to show the title or not
                        EditorGUILayout.BeginHorizontal();
                        EditorGUI.indentLevel = 0;

                        SerializedProperty cManEn = cManG.FindPropertyRelative("enabled");

                        if (TitleButton(new GUIContent(cManGC.FindPropertyRelative("name").stringValue, "Checkbox on the right enables the process group"), cManGC.FindPropertyRelative("expanded" + (post ? "P" : "T")), "BoldLabel") && cManEn.boolValue)
                        {//if group showing
                            ...
                            for (int p = 0; p < cManG.FindPropertyRelative("manufacture").arraySize; p++)
                            {//go through current manufacture processes
                               ...
                               new GUIContent(cManC.FindPropertyRelative("name").stringValue, controller.manufactureTooltips[m][p]));
                                    ...
                                    {//if enabled, allow times to be edited
                                        ...
                                        if (cMan.FindPropertyRelative("enabled").boolValue && !controller.expTraders.enabled)
                                        {//if enabled and not expendable, show price
                                            EditorGUILayout.BeginHorizontal();
        ...
        {//needs to check that there is something enabled in order to show the title
            for (int c = 0; c < check.arraySize; c++)
            {//go through all checking group
        ...
        {//the option GUI used for traders and spawners
            ...
            if (AnyGoods(item, "items"))
            {
                ...
                for (int g = 0; g < controllerNormal.goods.Count; g++)
                {
                    ...
                    if (itemGroup.arraySize > 0)
                    {//dont show anything if nothing in group
                        EditorGUI.indentLevel = 0;
                        ...
                        if (TitleButton(new GUIContent(currentGroup.FindPropertyRelative("name").stringValue), expanded, "BoldLabel"))
