...
{//use namespace to stop any name conflicts
		...
		public class ControllerEditor : Editor
	{
		...
		#region options
		private SerializedProperty selC;
		private SerializedProperty selG;
		private SerializedProperty selM;
		private SerializedProperty showGN;
		private SerializedProperty smallScroll;
		private SerializedProperty scrollPosS;
		private SerializedProperty scrollPosG;
		private SerializedProperty scrollPosM;
		private SerializedProperty scrollPosGG;
		private SerializedProperty scrollPosMG;
		#endregion
		...
		public void OnEnable ()
		{
			...
			for (int t = 0; t<controllerNormal.traders.Length; t++) {
				...
				traderScripts [t] = new SerializedObject (controllerNormal.traderScripts [t]);
						...
						#region get options
			selC = controllerSO.FindProperty ("selC");
			selG = controllerSO.FindProperty ("selGC");
			selM = controllerSO.FindProperty ("selMC");
			showGN = controllerSO.FindProperty ("showGN");
			smallScroll = controllerSO.FindProperty ("smallScroll");
			scrollPosS = controllerSO.FindProperty ("scrollPosS");
			scrollPosG = controllerSO.FindProperty ("scrollPosG");
			scrollPosM = controllerSO.FindProperty ("scrollPosM");
			scrollPosGG = controllerSO.FindProperty ("scrollPosGG");
			scrollPosMG = controllerSO.FindProperty ("scrollPosMG");
						#endregion
			...
			controllerNormal.SortTradePosts ();//sort out all of the trade posts
			controllerNormal.SortTraders ();//sort out all of the traders
		}//end OnEnable
		...
		public override void OnInspectorGUI ()
		{
			...
			//GUILayout.Toolbar()
			int selCB = selC.intValue;
			selC.intValue = GUILayout.Toolbar (selC.intValue, new string[] {
								"Settings",
								"Goods",
								"Manufacturing"
						});//show the toolbar
			if (selC.intValue != selCB)
				GUIUtility.keyboardControl = 0;
			GUILayout.FlexibleSpace ();//another space so is in the middle
			EditorGUILayout.EndHorizontal ();
		
			switch (selC.intValue) {
				#region settings
			case 0:
				GUITools.StartScroll (scrollPosS, smallScroll);
					#region general	
			...
			case 1:
				...
				scrollPosGG.vector2Value = EditorGUILayout.BeginScrollView (scrollPosGG.vector2Value, GUILayout.Height (40f));
				...
				int selGB = selG.intValue;
				selG.intValue = GUILayout.Toolbar (selG.intValue, namesG);
				if (selG.intValue != selGB)
					GUIUtility.keyboardControl = 0;
				...
				SerializedProperty currentGoodsGroup = goods.GetArrayElementAtIndex (selG.intValue).FindPropertyRelative ("goods");
				SerializedProperty goodName = goods.GetArrayElementAtIndex (selG.intValue).FindPropertyRelative ("name");
				EditorGUILayout.BeginHorizontal ();
				EditorGUILayout.PropertyField (goodName);
				if (goodName.stringValue == "")
					goodName.stringValue = "Goods group " + selG.intValue;

				GUI.enabled = goods.arraySize > 1;
				if (GUITools.PlusMinus (false)) {
					goods.DeleteArrayElementAtIndex (selG.intValue);
					GroupRemove (selG.intValue);
					if (selG.intValue > 0)
						selG.intValue--;
					GUIUtility.keyboardControl = 0;
					break;
				...
				if (GUILayout.Button (new GUIContent ("Sort", "Sort the goods by name alphabetically"), EditorStyles.miniButtonLeft, GUILayout.MinWidth (45f)))
					SortLists (selG.intValue);
				GUI.enabled = true;
				GUITools.ExpandCollapse (currentGoodsGroup, "expanded", true);
				EditorGUILayout.EndHorizontal ();
			
				GUITools.StartScroll (scrollPosG, smallScroll);
				...
				if (currentGoodsGroup.arraySize == 0 && GUILayout.Button ("Add good")) {//if there are no goods added, have one larger add button
					...
					EditLists (true, 0, selG.intValue, true);
				}
				...
				for (int g = 0; g<currentGoodsGroup.arraySize; g++) {//go through all goods, displaying them
					...
					EditorGUILayout.BeginHorizontal ();
					bool beforeG = currentGood.FindPropertyRelative ("expanded").boolValue;
					currentGood.FindPropertyRelative ("expanded").boolValue = GUILayout.Toggle (currentGood.FindPropertyRelative ("expanded").boolValue, currentGood.FindPropertyRelative ("name").stringValue, "Foldout");
					if (beforeG != currentGood.FindPropertyRelative ("expanded").boolValue)
						GUIUtility.keyboardControl = 0;
					...
					if (GUILayout.Button (new GUIContent ("▲", "Move up"), EditorStyles.miniButtonLeft)) {
						...
						MoveFromPoint (g - 1, selG.intValue, true);
						ListShuffle (true, g, g - 1, selG.intValue);
					}
					...
					if (GUILayout.Button (new GUIContent ("+", "Add good after"), EditorStyles.miniButtonMid)) {
						...
						EditLists (true, g + 1, selG.intValue, true);
						...
						MovePointsAfter (g, selG.intValue, false);
					...
					if (GUILayout.Button (new GUIContent ("-", "Remove good"), EditorStyles.miniButtonMid)) {
						EditLists (true, g, selG.intValue, false);
				
						currentGoodsGroup.DeleteArrayElementAtIndex (g);
						MovePointsAfter (g, selG.intValue, true);
						break;
					}
					...
					if (GUILayout.Button (new GUIContent ("▼", "Move down"), EditorStyles.miniButtonRight)) {
						currentGoodsGroup.MoveArrayElement (g, g + 1);
						MoveFromPoint (g + 1, selG.intValue, false);
						ListShuffle (true, g, g + 1, selG.intValue);
					}
			...
			case 2:
				...
				EditorGUILayout.BeginHorizontal ();//have a toolbar with the different group names on
				scrollPosMG.vector2Value = EditorGUILayout.BeginScrollView (scrollPosMG.vector2Value, GUILayout.Height (40f));
				...
				int selMB = selM.intValue;
				selM.intValue = GUILayout.Toolbar (selM.intValue, namesM);
										
				if (selM.intValue != selMB)
					GUIUtility.keyboardControl = 0;
				...
				SerializedProperty currentManGroup = manufacture.GetArrayElementAtIndex (selM.intValue).FindPropertyRelative ("manufacture");
				SerializedProperty manName = manufacture.GetArrayElementAtIndex (selM.intValue).FindPropertyRelative ("name");
				EditorGUILayout.BeginHorizontal ();
				EditorGUILayout.PropertyField (manName);
				if (manName.stringValue == "")
					manName.stringValue = "Manufacture group " + selM.intValue;

				GUI.enabled = manufacture.arraySize > 1;
				if (GUITools.PlusMinus (false)) {
					manufacture.DeleteArrayElementAtIndex (selM.intValue);
					GroupRemove (postScripts, "manufacture", selM.intValue);
					if (selM.intValue > 0)
						selM.intValue--;
					GUIUtility.keyboardControl = 0;
					break;
				...
				GUITools.StartScroll (scrollPosM, smallScroll);
				...
				if (currentManGroup.arraySize == 0 && GUILayout.Button ("Add process")) {//if there are no processes added, have one larger add button
					...
					EditLists (false, 0, selM.intValue, true);
				...
				for (int m = 0; m<currentManGroup.arraySize; m++) {
					...
					EditorGUILayout.BeginHorizontal ();
					bool beforeM = currentManufacture.FindPropertyRelative ("expanded").boolValue;
					currentManufacture.FindPropertyRelative ("expanded").boolValue = GUILayout.Toggle (currentManufacture.FindPropertyRelative ("expanded").boolValue, new GUIContent (currentManufacture.FindPropertyRelative ("name").stringValue, currentManufacture.FindPropertyRelative ("tooltip").stringValue), "Foldout");

					if (beforeM != currentManufacture.FindPropertyRelative ("expanded").boolValue)
						GUIUtility.keyboardControl = 0;
					GUILayout.FlexibleSpace ();//used so options are all at the end
					...
					if (GUILayout.Button (new GUIContent ("▲", "Move up"), EditorStyles.miniButtonLeft)) {
						...
						ListShuffle (false, m, m - 1, selM.intValue);
					}
					GUI.enabled = !Application.isPlaying;//set back to enabled if not playing
					if (GUILayout.Button (new GUIContent ("+", "Add process after"), EditorStyles.miniButtonMid)) {
						...
						EditLists (false, m + 1, selM.intValue, true);
					...
					if (GUILayout.Button (new GUIContent ("-", "Remove process"), EditorStyles.miniButtonMid)) {
						...
						EditLists (false, m, selM.intValue, false);
						break;
					}
					...
					if (currentManufacture.FindPropertyRelative ("expanded").boolValue) {
						...
						EditorGUILayout.LabelField (new GUIContent (ManMass (currentManufacture.FindPropertyRelative ("needing")) + " ► " + ManMass (currentManufacture.FindPropertyRelative ("making")),
						...
						if (GUITools.PlusMinus (true)) {//add needing element
							...
							controllerNormal.manufacture [selM.intValue].manufacture [m].needing.Add (new NeedMake ());
							controllerNormal.manufacture [selM.intValue].manufacture [m].needing [controllerNormal.manufacture [selM.intValue].manufacture [m].needing.Count - 1].number = 1;
						}
						EditorGUILayout.EndHorizontal ();
						...
						if (GUITools.PlusMinus (true)) {//add making element
							...
							controllerNormal.manufacture [selM.intValue].manufacture [m].making.Add (new NeedMake ());
							controllerNormal.manufacture [selM.intValue].manufacture [m].making [controllerNormal.manufacture [selM.intValue].manufacture [m].making.Count - 1].number = 1;
						}
						EditorGUILayout.EndHorizontal ();
		...
		void SortLists (int groupNo)
		{
			...
			for (int t = 0; t<controllerNormal.traderScripts.Length; t++) {//go through all traders
				AllowGroup groupAllow = controllerNormal.traderScripts [t].allowItems [groupNo];//allow group for the trader
				for (int a = 0; a<groupAllow.allowItems.Count; a++)//go through all allow
					groupAllow.allowItems [a].name = before [a].name;//and set the name to what it was before sorting
				groupAllow.allowItems.Sort ();
			}//end for all traders
		}//end SortLists
		...
		void GroupRemove (int groupNo)
		{
			...
			GroupRemove (postScripts, "stock", groupNo);
						...
						#region traders
			GroupRemove (traderScripts, "allowItems", groupNo);
						#endregion
		}//end GroupRemove
		...
		void ListShuffle (bool goods, int moveFrom, int moveTo, int groupNo)
		{
			...
			if (goods) {//only need to edit traders if it is goods that have been moved
				for (int t = 0; t<traderScripts.Length; t++) {//go through all traders
					traderScripts [t].Update ();
					traderScripts [t].FindProperty ("allowItems").GetArrayElementAtIndex (groupNo).FindPropertyRelative ("allowItems").MoveArrayElement (moveFrom, moveTo);
					traderScripts [t].ApplyModifiedProperties ();
				}//end for traders
			}//end goods shuffle check
		}//end ListShuffle
		...
		void EditLists (bool goods, int point, int groupNo, bool adding)
		{
			for (int p = 0; p<postScripts.Length; p++) {//go through all posts
				if (goods) {
					...
					if (adding) {
						...
						inserted.FindPropertyRelative ("minMaxS").boolValue = inserted.FindPropertyRelative ("minMaxE").boolValue = false;
					} else
			...
			if (goods) {//only need to edit traders if it is the goods list being edited
				for (int t = 0; t<traderScripts.Length; t++) {//go through all traders
					traderScripts [t].Update ();
					SerializedProperty allow = traderScripts [t].FindProperty ("allowItems").GetArrayElementAtIndex (groupNo).FindPropertyRelative ("allowItems");
					if (adding) {
						allow.InsertArrayElementAtIndex (point);
						allow.GetArrayElementAtIndex (point).FindPropertyRelative ("enabled").boolValue = true;
					} else
						allow.DeleteArrayElementAtIndex (point);
					traderScripts [t].ApplyModifiedProperties ();
				}//end for all traders
			}//end goods change check
		}//end EditLists
		...
		void AddGoodsGroup (int loc)
		{
			...
			for (int t = 0; t<traderScripts.Length; t++) {//go through traders, adding
				traderScripts [t].Update ();
				SerializedProperty allow = traderScripts [t].FindProperty ("allowItems");
				allow.InsertArrayElementAtIndex (loc);
				allow.GetArrayElementAtIndex (loc).FindPropertyRelative ("allowItems").arraySize = 0;
				traderScripts [t].ApplyModifiedProperties ();
			}//end for traders
		...
		{
			for (int p = 0; p<postScripts.Length; p++) {//go through posts, adding
				SerializedProperty postMan = postScripts [p].FindProperty ("manufacture");
				postMan.InsertArrayElementAtIndex (loc);
				postMan.GetArrayElementAtIndex (loc).FindPropertyRelative ("manufacture").arraySize = 0;
			}//end for posts
		...
		void CheckManufacturing (int manufactureCount, int[] groupLengthsM, int[] groupLengthsG, SerializedProperty manufacture)
		{
			...
			for (int m1 = 0; m1<manufacture.arraySize; m1++) {//go through all manufacture groups
				...
				for (int m2 = 0; m2<processCount; m2++) {//gro through all processes
					...
					for (int p = 0; p<controllerNormal.postScripts.Length; p++) {//go through all posts
						TradePost post = controllerNormal.postScripts [p];
						PostMnfctr man = post.manufacture [m1].manufacture [m2];
		...
		{
			SerializedProperty expanded = options.FindPropertyRelative ("expanded");
				...
				if (enabled.boolValue) {//show only if enabled		
					...
					if (GUITools.PlusMinus (true)) {
						...
						for (int p = 0; p<postScripts.Length; p++)//add the new element to all trade posts
							postScripts [p].FindProperty (plural).FindPropertyRelative ("enabled").InsertArrayElementAtIndex (number);
					...
					if (number > 0) {//check that there are options to show
						...
						for (int o = 0; o<number; o++) {//for all options
							...
							if (GUITools.PlusMinus (false)) {
								for (int p = 0; p<postScripts.Length; p++) 
									postScripts [p].FindProperty (plural).FindPropertyRelative ("enabled").DeleteArrayElementAtIndex (o);
								if (option == 1) {//only need to update trader information if factions are being edited
									for (int t = 0; t<traderScripts.Length; t++) {
										traderScripts [t].Update ();
		...
		/// Return the total mass of the items in the needing or making
		string ManMass (SerializedProperty curManNM)
		{
			float mass = 0;
			for (int n = 0; n<curManNM.arraySize; n++) {//go through all needing / making
				SerializedProperty cNM = curManNM.GetArrayElementAtIndex (n);
				if (cNM.FindPropertyRelative ("itemID").intValue >= 0)//check that item is not undefined
					mass += controllerNormal.goods [cNM.FindPropertyRelative ("groupID").intValue].goods [cNM.FindPropertyRelative ("itemID").intValue].mass * cNM.FindPropertyRelative ("number").intValue;
			}//end for needing / making
			return UnitString (mass, false);
		}//end ManMass
