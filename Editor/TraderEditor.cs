        ...
        public override void OnInspectorGUI()
        {
            ...
            switch (sel)
            {
                ...
                case 0:
                    ...
                    #region options
                    if (GUITools.TitleGroup(new GUIContent("Options", "Set general information for the trader which will affect trading"), controllerSO.FindProperty("opT"), false))
                    {//if showing options

                        if (!expendable)
                        {//if not expendable, then show target post
                            ...
                            if (GUILayout.Button(new GUIContent("Find post", "Attempts to find the trade post if one is within the close distance value"), EditorStyles.miniButtonLeft, GUILayout.MaxWidth(GUI.skin.button.CalcSize(new GUIContent("Find post")).x)))
                            {
                                ...
                                {//go through all posts
                                    if (Vector3.Distance(traderNormal.transform.position, posts[p].transform.position) <= closeDistance.floatValue)
                                    {//check close enough
                        ...
                        if (controllerNormal.pickUp)
                        {//only need to show this if is enabled in the controller
