...
{//use namespace to stop any name conflicts
    ...
    public class Trader : MonoBehaviour
    {
        ...
        {//called when the trader has reached the post, so needs to sell items
            ...
            {//go through group
                for (int i = 0; i < items[g].items.Count; i++)
                {//go through items
                    ...
                    if (stock.buy)
                    {//make sure post allowed to buy the item
        ...
        {//go through the manufacturing processes, and if not being made, check that can
            if (!expendable)
            {//if expendable, dont manufacture anything
        ...
        public void ChangeTraderHome(GameObject post)
        ...
        {//drop all of the cargo. is used here so can be called when destroyed, but could be used for jetissoing all cargo
            for (int g = 0; g < items.Count; g++)
            {//for all groups
                for (int i = 0; i < items[g].items.Count; i++)
                {//for all items
                    ...
                    if (number > 0)
                    {//if there is cargo to drop
                        if (dropCargo)
                        {//check can drop cargo
        ...
        public void DropCargo(int number, int groupID, int itemID)
